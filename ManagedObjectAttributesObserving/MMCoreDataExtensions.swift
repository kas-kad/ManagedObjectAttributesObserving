...
extension Updatable where Self: NSManagedObject {
	...
	static func MM_batchUpdate(propertiesToUpdate: [AnyHashable: Any], predicate: NSPredicate?, inContext ctx: NSManagedObjectContext) -> NSBatchUpdateResult? {
		...
		do {
			...
			result = nil
			print("[Core Data] batch update error: \(error)")
		}
...
extension NSManagedObject {
	...
	func MM_deleteEntityInContext(_ context: NSManagedObjectContext) -> Bool {
		do {
			...
			print("An error occured while deleting an object \(self): \(error)")
		}
		return false
	}
}
...
extension NSManagedObjectContext {
	...
	func MM_saveWithOptions(_ options: MMContextSaveOptions, completion: ((Bool, NSError?) -> Void)?) {
		...
		if hasChanges == false {
			print("NO CHANGES IN ** \(MM_workingName) ** CONTEXT - NOT SAVING")
			if (saveParentContexts && parent != nil) {
				print("Proceeding to save parent context \(String(describing: parent?.MM_description))")
			} else {
				completion?(true, nil)
				return
			}
		...
		let saveBlock = {
			...
			print("Objects - Inserted \(numberOfInsertedObjects), Updated \(numberOfUpdatedObjects), Deleted \(numberOfDeletedObjects)")
			
			print("→ Saving \(self.MM_description) [\(optionsSummary)]")
			...
			do {
				...
				error = err
				print("Unable to perform save: \(err)")
			} catch {
				print("Unable to perform save. Unknown exception.")
			}
			defer {
				if saved == false {
					...
					if let parentCtx = self.parent , saveParentContexts {
						...
						print("→ Finished saving: \(self.MM_description)")
						completion?(saved, error)
					}
				}
			}
	...
	class func MM_removePersistentStoreFilesAtURL(_ url: URL) -> Bool {
		...
		for fileURL in [url, shmSidecar, walSidecar] {
			do {
				...
				print("An error occured while deleting persistent store files: \(error)")
			}
		}
