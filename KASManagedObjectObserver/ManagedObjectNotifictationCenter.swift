//
//  ManagedObjectNotifictationCenter.swift
...
class ManagedObjectNotificationCenter {
	...
	init() {
		self.observersQueue = DispatchQueue(label: "kas.queue.serial.managedobjectnotificationcenter")
	}
...
class Observation: NSObject {
	...
	override var hash: Int {
		...
		return (String(describing: address(o: observer)) + String(describing: address(o: observee)) + keyPath).hashValue
	}
...
extension NSManagedObject {
	...
	func kas_willTurnIntoFault() {
		self.kas_willTurnIntoFault()
...
fileprivate extension NSObject {
	fileprivate struct AssociatedKeys {
		static var AssociatedDeallocationWatchersKey = "kas_AssociatedDeallocationWatchersKey"
		static var AssociatedFaultingWatchersKey = "kas_AssociatedFaultingWatchersKey"
	}
	...
	override open class func initialize() {
		struct Static {
			static let swizzling : () = {
				...
				let swizzledSelector = #selector(NSManagedObject.kas_willTurnIntoFault)
